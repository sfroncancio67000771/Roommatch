...
const API_URL_ALOJAMIENTOS = 'http://localhost:8081/api/v1/alojamiento/filtrar';
        const UPLOAD_URL = 'http://localhost:8081/api/v1/fotos/upload';

        // Función para obtener los IDs de alojamiento y cargarlos en el select
        async function cargarAlojamientos() {
            try {
                ...
                const select = document.getElementById('idAlojamiento');
                
                alojamientos.forEach(alojamiento => {
                    const option = document.createElement('option');
                    option.value = alojamiento.idAlojamiento;
                    option.textContent = `${alojamiento.idAlojamiento} - ${alojamiento.nombreAlojamiento}`;
                    select.appendChild(option);
                });
            } catch (error) {
                console.error('Error al cargar los alojamientos:', error);
                document.getElementById('error-message').textContent = 'Error al cargar los alojamientos';
            }
        }
        ...
        document.getElementById('uploadForm').addEventListener('submit', async (event) => {
            ...
            formData.append('idAlojamiento', document.getElementById('idAlojamiento').value);
            ...
            try {
                ...
                document.getElementById('error-message').textContent = 'Error al subir la imagen';
            }
        });

        // Cargar los alojamientos al cargar la página
        window.onload = cargarAlojamientos;

        const uploadArea = document.getElementById('upload-area');
        const fileInput = document.getElementById('archivo');

        // Evento para abrir el selector de archivos al hacer clic en el área de subida
        uploadArea.addEventListener('click', () => {
            fileInput.click();
        });

        // Evento para mostrar el nombre del archivo cuando se selecciona
        fileInput.addEventListener('change', (event) => {
            const fileName = event.target.files[0].name;
            uploadArea.querySelector('.upload-text').textContent = `Archivo seleccionado: ${fileName}`;
        });

        // Funcionalidad de arrastrar y soltar
        uploadArea.addEventListener('dragover', (event) => {
            event.preventDefault();
            uploadArea.style.backgroundColor = '#e0f7f7';
            uploadArea.style.borderColor = '#007373';
        });
        ...
        uploadArea.addEventListener('drop', (event) => {
            ...
            fileInput.files = event.dataTransfer.files; // Asigna el archivo al input
            uploadArea.querySelector('.upload-text').textContent = `Archivo seleccionado: ${file.name}`;
            uploadArea.style.backgroundColor = '';
            uploadArea.style.borderColor = '#008080';
        });
