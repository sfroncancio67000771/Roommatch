const BASE_URL = 'http://localhost:8081/api/v1/fotos/'; // Asegúrate de que esta URL sea correcta
const API_URL = 'http://localhost:8081/api/v1/alojamiento/filtrar'; // Asegúrate de que esta URL sea correcta
...
async function mostrarAlojamientos(alojamientos) {
    ...
    try {
        for (const alojamiento of alojamientos) {
            ...
            const imagenUrl = await obtenerFoto(alojamiento.idAlojamiento); // Asegúrate de usar el campo correcto
            ...
            if (imagenUrl) {
                ...
                // Asigna una imagen por defecto si no hay imagen
                imgElement.src = 'ruta/a/imagen/por/defecto.jpg'; // Cambia a la ruta de tu imagen por defecto
            }
...
function contactar(idAlojamiento) {
    window.location.href = `detalle_habitacionp.html?id=${idAlojamiento}`;
}
